...
def get_recall_each_class(labels, preds):
	...
	for i, name in enumerate(LABELS):
		...
		b = np.sum(labels==i)
		rec = a * 1.0 / b
		print("{} : recall {:.4f}".format(name, rec))

def get_precision_each_class(labels, preds):
	print("Precision for each class")
	for i, name in enumerate(LABELS):
		a = np.sum(np.logical_and(labels==i, preds==i))
		b = np.sum(preds==i)
		prec = a * 1.0 / b
		print("{} : precision {:.4f}".format(name, prec))
...
class HARPredictor(Predictor):
	...
	def _predict(self):
		...
		print("F1 Score {:.4f}, Acc {:.4f}, Precision {:.4f}, Recall {:.4f}".
			format(f1, acc, pres, rec))
		# get_recall_each_class(labels, preds)
		# get_precision_each_class(labels, preds)
